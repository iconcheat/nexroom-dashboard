...
function Card({label,value,highlight=false}:{label:string;value:string|number;highlight?:boolean;}){
  return(
    <div className={['relative w-full max-w-full rounded-2xl p-4 flex flex-col gap-1.5 min-w-0',
      ...
      <div className={[highlight
        ...
        'font-extrabold leading-[1.05] tracking-tight tabular-nums whitespace-normal md:whitespace-nowrap min-w-0 pr-0.5'].join(' ')}
      ...
      {highlight&&(<span className="pointer-events-none hidden md:block absolute inset-y-0 -left-24 md:-left-36 w-24 md:w-32 bg-gradient-to-r from-transparent via-white/50 to-transparent blur-md md:blur-lg" style={{animation:'shine 3.6s linear infinite'}}/>)}
    </div>
  );
}