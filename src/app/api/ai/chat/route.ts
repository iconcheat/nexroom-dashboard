...
export async function POST(req: NextRequest) {
  try {
    ...
    const ctx = {
      ...(body?.context || {}),
      staff_id:   body?.context?.staff_id   ?? headersLower['x-staff-id'] ?? cookies.get('staff_id')?.value ?? null,
      dorm_id:    body?.context?.dorm_id    ?? headersLower['x-dorm-id']  ?? cookies.get('dorm_id')?.value ?? null,
      dorm_name:  body?.context?.dorm_name  ?? cookies.get('dorm_name')?.value ?? null,
      role:       body?.context?.role       ?? cookies.get('role')?.value ?? 'staff',
      session_id: body?.context?.session_id ?? headersLower['x-session-id'] ?? cookies.get('nxr_session')?.value ?? '',
      channel:    'dashboard',
    ...
    const payload = { ...body, context: ctx };
    ...
    // ส่งผลแรกจาก n8n (เช่น ข้อความ + ปุ่ม) ตรงกลับไป UI
